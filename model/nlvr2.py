...
class UniterForNlvr2PairedAttn(UniterPreTrainedModel):
    ...
    def __init__(self, config, img_dim):
        ...
        self.attn_pool = AttentionPool(
            config.hidden_size, config.attention_probs_dropout_prob
        )
    ...
    def forward(self, batch, compute_loss=True):
        ...
        left_out, right_out = (
            sequence_output.contiguous().view(bs // 2, tl * 2, d).chunk(2, dim=1)
        )
        ...
        l2r_attn, _ = self.attn1(
            left_out, right_out, right_out, key_padding_mask=right_mask
        )
        r2l_attn, _ = self.attn2(
            right_out, left_out, left_out, key_padding_mask=left_mask
        )
