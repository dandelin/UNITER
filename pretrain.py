...
def build_dataloader(dataset, collate_fn, is_train, opts):
    ...
    sampler = TokenBucketSampler(
        dataset.lens, bucket_size=BUCKET_SIZE, batch_size=batch_size, droplast=is_train
    )
    loader = DataLoader(
        dataset,
...
def build_dataloader_itm(dataset, collate_fn, is_train, opts):
    ...
    sampler = TokenBucketSamplerForItm(
        dataset, bucket_size=BUCKET_SIZE, batch_size=batch_size, droplast=is_train
    )
    loader = DataLoader(
        dataset,
        batch_sampler=sampler,
...
def build_mrfr_dataset(txt_db, img_db, is_train, opts):
    if is_train:
        datasets = [MrfrDataset(opts.mrm_prob, t, i) for t, i in zip(txt_db, img_db)]
        dataset = ConcatDatasetWithLens(datasets)
    else:
...
def build_mrc_dataset(txt_db, img_db, is_train, opts):
    if is_train:
        datasets = [MrcDataset(opts.mrm_prob, t, i) for t, i in zip(txt_db, img_db)]
        dataset = ConcatDatasetWithLens(datasets)
    else:
...
def create_dataloaders(datasets, is_train, opts, all_img_dbs=None):
    if all_img_dbs is None:
        all_img_dbs = ImageLmdbGroup(
            opts.conf_th, opts.max_bb, opts.min_bb, opts.num_bb, opts.compressed_db
        )
    dataloaders = {}
    for dset in datasets:
        ...
        for i, t in enumerate(dset["tasks"]):
            ...
            if is_train:
                LOGGER.info(
                    f"Loading {task} train dataset "
                    f"{dset['db']}, {[img.img_dir for img in img_db]}"
                )
                txt_db = [TxtTokLmdb(path, opts.max_txt_len) for path in dset["db"]]
            else:
                LOGGER.info(
                    f"Loading {task} validation dataset, "
...
def main(opts):
    ...
    assert all(
        tokenizer == json.load(open(f"{db}/meta.json"))["bert"] for db in all_dbs
    )

    # build data loaders
    train_dataloaders, all_img_dbs = create_dataloaders(opts.train_datasets, True, opts)
    val_dataloaders, _ = create_dataloaders(opts.val_datasets, False, opts, all_img_dbs)
    ...
    model, optimizer = amp.initialize(
        model, optimizer, num_losses=len(task2scaler), enabled=opts.fp16, opt_level="O2"
    )
    ...
    if opts.itm_ot_lambda > 0:
        for task in train_dataloaders.keys():
            if task.startswith("itm"):
                ...
                task2loss[f"{task}_ot_pos"] = RunningMeter(f"loss/{task}_ot_pos")
                task2loss[f"{task}_ot_neg"] = RunningMeter(f"loss/{task}_ot_neg")
    ...
    for step, (name, batch) in enumerate(meta_loader):
        ...
        if (step + 1) % opts.gradient_accumulation_steps == 0:
            ...
            TB_LOGGER.log_scaler_dict(
                {l.name: l.val for l in task2loss.values() if l.val is not None}
            )
            TB_LOGGER.step()
...
def validate(model, val_dataloaders):
    ...
    for task, loader in val_dataloaders.items():
        ...
        TB_LOGGER.log_scaler_dict({f"valid_{task}/{k}": v for k, v in val_log.items()})
    model.train()
...
def validate_mrfr(model, val_loader):
    ...
    LOGGER.info(
        f"validation finished in {int(tot_time)} seconds, " f"loss: {val_loss:.2f}"
    )
    return val_log
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--compressed_db", action="store_true", help="use compressed LMDB"
    )
    ...
    parser.add_argument(
        "--max_bb", type=int, default=100, help="max number of bounding boxes"
    )
    parser.add_argument(
        "--min_bb", type=int, default=10, help="min number of bounding boxes"
    )
    parser.add_argument(
        "--num_bb", type=int, default=36, help="static number of bounding boxes"
    )
    ...
    parser.add_argument(
        ...
        default=4096,
        type=int,
        help="Total batch size for training. " "(batch by tokens)",
    )
    parser.add_argument(
        "--val_batch_size",
        default=4096,
