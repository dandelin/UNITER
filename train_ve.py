...
def create_dataloader(
    img_path, txt_path, batch_size, is_train, dset_cls, collate_fn, opts
):
    ...
    sampler = TokenBucketSampler(
        dset.lens, bucket_size=BUCKET_SIZE, batch_size=batch_size, droplast=is_train
    )
    loader = DataLoader(
        dset,
...
def main(opts):
    ...
    # train_examples = None
    LOGGER.info(f"Loading Train Dataset {opts.train_txt_db}, " f"{opts.train_img_db}")
    ...
    if rank == 0:
        ...
        pickle.dump(
            ans2label, open(join(opts.output_dir, "ckpt", "ans2label.pkl"), "wb")
        )
    ...
    if opts.num_train_steps % opts.valid_steps != 0:
        for split, loader in [("val", val_dataloader), ("test", test_dataloader)]:
            LOGGER.info(
                f"Step {global_step}: start running " f"validation on {split} split..."
            )
            val_log, results = validate(model, loader, label2ans, split)
            with open(
                f"{opts.output_dir}/results/"
...
def validate(model, val_loader, label2ans, split="val"):
    ...
    for i, batch in enumerate(val_loader):
        ...
        answers = [
            label2ans[i] for i in scores.max(dim=-1, keepdim=False)[1].cpu().tolist()
        ]
        qids = batch["qids"]
...
if __name__ == "__main__":
    ...
    # Required parameters
    parser.add_argument(
        "--train_txt_db", default=None, type=str, help="The input train corpus. (LMDB)"
    )
    parser.add_argument(
        "--train_img_db", default=None, type=str, help="The input train images."
    )
    parser.add_argument(
        "--val_txt_db",
        default=None,
    ...
    parser.add_argument(
        "--val_img_db", default=None, type=str, help="The input validation images."
    )
    parser.add_argument(
        "--test_txt_db", default=None, type=str, help="The input test corpus. (LMDB)"
    )
    parser.add_argument(
        "--test_img_db", default=None, type=str, help="The input test images."
    )
    parser.add_argument(
