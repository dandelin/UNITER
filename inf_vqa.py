...
def main(opts):
    ...
    val_log, results, logits = evaluate(
        model, eval_dataloader, label2ans, opts.save_logits
    )
    ...
    if hvd.rank() == 0:
        with open(f"{result_dir}/" f"results_{opts.checkpoint}_all.json", "w") as f:
            json.dump(all_results, f)
        if opts.save_logits:
            np.savez(f"{result_dir}/logits_{opts.checkpoint}_all.npz", **all_logits)
...
def evaluate(model, eval_loader, label2ans, save_logits=False):
    ...
    for i, batch in enumerate(eval_loader):
        ...
        if i % 100 == 0 and hvd.rank() == 0:
            ...
            LOGGER.info(f"{n_results}/{len(eval_loader.dataset)} " "answers predicted")
        n_ex += len(qids)
    ...
    model.train()
    LOGGER.info(
        f"evaluation finished in {int(tot_time)} seconds "
...
if __name__ == "__main__":
    ...
    # Required parameters
    parser.add_argument(
        "--txt_db", default=None, type=str, help="The input train corpus. (LMDB)"
    )
    parser.add_argument(
        "--img_db", default=None, type=str, help="The input train images."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--batch_size", default=8192, type=int, help="number of tokens in a batch"
    )
