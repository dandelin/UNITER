...
def main(opts):
    if not exists(opts.output):
        ...
        raise ValueError(
            "Found existing DB. Please explicitly remove " "for re-processing"
        )
    meta = vars(opts)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--annotation", required=True, help="annotation JSON")
    parser.add_argument(
        "--missing_imgs", help="some training image features are corrupted"
    )
    parser.add_argument("--output", required=True, help="output dir of DB")
    parser.add_argument(
        "--toker", default="bert-base-cased", help="which BERT tokenizer to used"
    )
