...
def build_optimizer(model, opts):
    ...
    param_optimizer = [
        (n, p) for n, p in model.named_parameters() if "vcr_output" not in n
    ]
    param_top = [(n, p) for n, p in model.named_parameters() if "vcr_output" in n]
...
def load_img_feat(db_list, all_img_dbs, opts):
    ...
    if gt_db_path != "":
        img_db_gt = DetectFeatLmdb(
            gt_db_path, -1, opts.max_bb, opts.min_bb, 100, opts.compressed_db
        )
...
def main(opts):
    ...
    val_final_dataloader = build_dataloader(
        val_final_dataset, vcr_eval_collate, False, opts
    )
...
def validate(model, val_loader):
    ...
    for i, batch in enumerate(val_loader):
        ...
        if scores.shape[1] > 8:
            qar_index = [
                4 + answer_ind.item() * 4 + i
                for answer_ind in qa_targets
                for i in range(4)
            ]
