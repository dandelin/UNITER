...
class Nlvr2PairedDataset(DetectFeatTxtTokDataset):
    def __init__(self, txt_db, img_db, use_img_type=True):
        ...
        self.lens = [
            2 * tl + sum(self.img_db.name2nbb[img] for img in txt2img[id_])
...
def nlvr2_paired_collate(inputs):
    (input_ids, img_feats, img_pos_feats, attn_masks, img_type_ids) = map(
        list, unzip(concat(outs for outs, _ in inputs))
    )
    ...
    if img_type_ids[0] is None:
        ...
        img_type_ids = pad_sequence(img_type_ids, batch_first=True, padding_value=0)
...
class Nlvr2TripletDataset(DetectFeatTxtTokDataset):
    def __init__(self, txt_db, img_db, use_img_type=True):
        ...
        txt2img = txt_db.txt2img
        self.lens = [
            tl + sum(self.img_db.name2nbb[img] for img in txt2img[id_])
            for tl, id_ in zip(txt_lens, self.ids)
        ]
    ...
    def __getitem__(self, i):
        ...
        return (input_ids, img_feat, img_pos_feat, attn_masks, img_type_ids, target)


def nlvr2_triplet_collate(inputs):
    (input_ids, img_feats, img_pos_feats, attn_masks, img_type_ids, targets) = map(
        list, unzip(inputs)
    )
