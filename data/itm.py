...
class ItmDataset(DetectFeatTxtTokDataset):
    ...
    def new_epoch(self):
        ...
        self.labels = np.random.choice(
            [0, 1], size=len(self.ids), p=[self.neg_sample_p, 1 - self.neg_sample_p]
        )
...
def _compute_ot_scatter(txt_lens, max_txt_len, joint_len):
    ot_scatter = (
        torch.arange(0, joint_len, dtype=torch.long)
        .unsqueeze(0)
        .repeat(len(txt_lens), 1)
    )
    for i, tl in enumerate(txt_lens):
        max_ind = max_txt_len + (joint_len - tl)
        ot_scatter.data[i, tl:] = torch.arange(
            max_txt_len, max_ind, dtype=torch.long
        ).data
    return ot_scatter
...
class ItmRankDataset(DetectFeatTxtTokDataset):
    def __init__(self, txt_db, img_db, neg_sample_size=1):
        assert neg_sample_size > 0, "ItmRankDataset need at least 1 negative sample"
    ...
    def __getitem__(self, i):
        ...
        id_pairs.extend(
            [(gt_txt_id, neg_img_id) for neg_img_id in neg_sample_img_ids]
            + [(neg_txt_id, gt_img_fname) for neg_txt_id in neg_sample_txt_ids]
        )
...
def itm_rank_collate(inputs):
    (input_ids, img_feats, img_pos_feats, attn_masks,) = map(
        list, unzip(concat(i for i in inputs))
    )
...
class ItmRankDatasetHardNegFromText(DetectFeatTxtTokDataset):
    ...
    def __getitem__(self, i):
        ...
        gather_index = get_gather_index(
            [tl] * len(img_ids), num_bbs, len(img_ids), tl, out_size
        )
...
class ItmRankDatasetHardNegFromImage(DetectFeatTxtTokDataset):
    ...
    def __getitem__(self, i):
        ...
        for txt_id in txt_ids:
            input_ids = self.txt_db.combine_inputs(self.txt_db[txt_id]["input_ids"])
        ...
        out_size = attn_masks.size(1)
        gather_index = get_gather_index(
            txt_lens, [nbb] * len(txt_ids), len(txt_ids), tl, out_size
        )
...
class ItmValDataset(DetectFeatTxtTokDataset):
    ...
    def _get_batch_ids(self, i):
        ...
        if neg_end > len(self.all_img_ids):
            ...
            neg_img_ids = self.all_img_ids[neg_st:] + self.all_img_ids[:neg_end]
        else:
            neg_img_ids = self.all_img_ids[neg_st:neg_end]

        assert len(neg_img_ids) == (self.bs - 1), "Did not sample enough neg samples"
...
class ItmEvalDataset(ItmValDataset):
    def __init__(self, *args, **kwargs):
        ...
        self.all_img_ids = sorted(
            copy.deepcopy(self.all_img_ids), key=lambda i: self.img_db.name2nbb[i]
        )

    def __getitem__(self, i):
        mini_batches = []
        for st in range(0, len(self.all_img_ids), self.bs):
            mini_batches.append(self.get_batch(i, self.all_img_ids[st : st + self.bs]))
        return mini_batches
