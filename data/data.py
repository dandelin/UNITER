...
class DetectFeatLmdb(object):
    def __init__(
        self, img_dir, conf_th=0.2, max_bb=100, min_bb=10, num_bb=36, compress=True
    ):
    ...
    def _compute_nbb(self):
        ...
        for fname in tqdm(fnames, desc="reading images"):
            ...
            if self.compress:
                ...
                confs = img_dump["conf"]
            name2nbb[fname] = compute_num_bb(
                confs, self.conf_th, self.min_bb, self.max_bb
            )
...
class TxtLmdb(object):
    def __init__(self, db_dir, readonly=True):
        ...
        if readonly:
            # training
            self.env = lmdb.open(
                db_dir, readonly=True, create=False, readahead=not _check_distributed()
            )
            self.txn = self.env.begin(buffers=True)
            self.write_cnt = None
        else:
            # prepro
            self.env = lmdb.open(
                db_dir, readonly=False, create=True, map_size=4 * 1024 ** 4
            )
    ...
    def __setitem__(self, key, value):
        ...
        ret = self.txn.put(
            key.encode("utf-8"), compress(msgpack.dumps(value, use_bin_type=True))
        )
        self.write_cnt += 1
...
class DetectFeatTxtTokDataset(Dataset):
    def __init__(self, txt_db, img_db):
        ...
        self.lens = [
            tl + self.img_db.name2nbb[txt2img[id_]]
...
def get_gather_index(txt_lens, num_bbs, batch_size, max_len, out_size):
    ...
    gather_index = (
        torch.arange(0, out_size, dtype=torch.long,).unsqueeze(0).repeat(batch_size, 1)
    )

    for i, (tl, nbb) in enumerate(zip(txt_lens, num_bbs)):
        gather_index.data[i, tl : tl + nbb] = torch.arange(
            max_len, max_len + nbb, dtype=torch.long
        ).data
    return gather_index
...
class ConcatDatasetWithLens(ConcatDataset):
    ...
    def _run_method_on_all_dsets(self, name):
        def run_all(*args, **kwargs):
            return [
                dset.__getattribute__(name)(*args, **kwargs) for dset in self.datasets
            ]
    ...
    def __getitem__(self, path):
        ...
        if img_db is None:
            img_db = DetectFeatLmdb(
                path, self.conf_th, self.max_bb, self.min_bb, self.num_bb, self.compress
            )
        return img_db
