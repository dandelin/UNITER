...
class VcrTxtTokLmdb(TxtTokLmdb):
    def __init__(self, db_dir, max_txt_len=120, task="qa,qar"):
        assert (
            task == "qa" or task == "qar" or task == "qa,qar"
        ...
        if task == "qa,qar":
            ...
            id2len_task = task
        if max_txt_len == -1:
            self.id2len = json.load(open(f"{db_dir}/id2len_{id2len_task}.json"))
        else:
            self.id2len = {
                id_: len_
...
class VcrDetectFeatTxtTokDataset(DetectFeatTxtTokDataset):
    def __init__(self, txt_db, img_db_gt=None, img_db=None):
        assert not (
            img_db_gt is None and img_db is None
        ...
        if self.img_db and self.img_db_gt:
            self.lens = [
                tl
                + self.img_db_gt.name2nbb[txt2img[id_][0]]
                + self.img_db.name2nbb[txt2img[id_][1]]
                for tl, id_ in zip(txt_lens, self.ids)
            ]
        elif self.img_db:
            self.lens = [
                tl + self.img_db.name2nbb[txt2img[id_][1]]
                for tl, id_ in zip(txt_lens, self.ids)
            ]
        else:
            self.lens = [
                tl + self.img_db_gt.name2nbb[txt2img[id_][0]]
                for tl, id_ in zip(txt_lens, self.ids)
            ]
...
class VcrDataset(VcrDetectFeatTxtTokDataset):
    ...
    def __getitem__(self, i):
        ...
        input_ids_q, input_ids_for_choices, type_ids_q = self._get_input_ids(example)
        ...
        for index, input_ids_a in enumerate(input_ids_for_choices):
            ...
            txt_type_ids = (
                [0] + type_ids_q + [type_id_for_choice] * (len(input_ids_a) + 2)
            )
            ...
            outs.append(
                (input_ids, txt_type_ids, img_feat, img_pos_feat, attn_masks, target)
            )
...
def vcr_collate(inputs):
    (input_ids, txt_type_ids, img_feats, img_pos_feats, attn_masks, targets) = map(
...
class VcrEvalDataset(VcrDetectFeatTxtTokDataset):
    ...
    def _get_input_ids(self, txt_dump):
        ...
        for index, input_ids_a in enumerate(input_ids_as):
            ...
            curr_type_ids_qa = [0] + type_ids_q + [2] * (len(input_ids_a) + 2)
        ...
        for index, input_ids_a in enumerate(input_ids_as):
            ...
            if (
                self.split == "val" and index == txt_dump["qa_target"]
            ) or self.split == "test":
                for input_ids_r in input_ids_rs:
                    curr_input_ids_qar = (
                        copy.deepcopy(curr_input_ids_qa)
                        + input_ids_r
                        + [self.txt_db.sep]
                    )
                    curr_type_ids_qar = copy.deepcopy(curr_type_ids_qa) + [3] * (
                        len(input_ids_r) + 2
                    )
...
def vcr_eval_collate(inputs):
    (input_ids, txt_type_ids, img_feats, img_pos_feats, attn_masks) = map(
    ...
    qa_targets = torch.stack([t for _, _, t, _ in inputs], dim=0)
    qar_targets = torch.stack([t for _, _, _, t in inputs], dim=0)
