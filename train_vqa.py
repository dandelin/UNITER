...
def build_optimizer(model, opts):
    ...
    param_optimizer = [
        (n, p) for n, p in model.named_parameters() if "vqa_output" not in n
    ]
    param_top = [(n, p) for n, p in model.named_parameters() if "vqa_output" in n]
...
def main(opts):
    ...
    ans2label = json.load(
        open(f"{dirname(abspath(__file__))}" f"/utils/ans2label.json")
    )
    ...
    model = UniterForVisualQuestionAnswering.from_pretrained(
        opts.model_config, checkpoint, img_dim=IMG_DIM, num_answer=len(ans2label)
    )
    model.to(device)
    ...
    if opts.num_train_steps % opts.valid_steps != 0:
        val_log, results = validate(model, val_dataloader, label2ans)
        with open(
            f"{opts.output_dir}/results/" f"results_{global_step}_" f"rank{rank}.json",
            "w",
        ) as f:
            json.dump(results, f)
        TB_LOGGER.log_scaler_dict(val_log)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--lr_mul", default=10.0, type=float, help="multiplier for top layer lr"
    )
    parser.add_argument(
