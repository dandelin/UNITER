...
def main(opts):
    ...
    if opts.keep_all:
        ...
        if opts.conf_th == -1:
            ...
            db_name = f"feat_th{opts.conf_th}_max{opts.max_bb}" f"_min{opts.min_bb}"
    if opts.compress:
        db_name += "_compressed"
    ...
    load = load_npz(
        opts.conf_th, opts.max_bb, opts.min_bb, opts.num_bb, keep_all=opts.keep_all
    )
    name2nbb = {}
    with mp.Pool(opts.nproc) as pool, tqdm(total=len(files)) as pbar:
        for i, (fname, features, nbb) in enumerate(
            ...
            pbar.update(1)
        txn.put(
            key=b"__keys__", value=json.dumps(list(name2nbb.keys())).encode("utf-8")
        )
        txn.commit()
        env.close()
    if opts.conf_th != -1 and not opts.keep_all:
        with open(
            ...
            f"max{opts.max_bb}_min{opts.min_bb}.json",
            "w",
        ) as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--nproc", type=int, default=8, help="number of cores used")
    parser.add_argument("--compress", action="store_true", help="compress the tensors")
    parser.add_argument(
        "--keep_all",
        action="store_true",
    ...
    parser.add_argument(
        "--num_bb", type=int, default=100, help="number of bounding boxes (fixed)"
    )
