...
def save_for_submission(pred_file):
    with open(os.path.join(pred_file), "r") as f:
        ...
        probs_grp = []
        ids_grp = []
        ordered_data = sorted(data.items(), key=lambda item: int(item[0].split("-")[1]))
...
def main(opts):
    ...
    eval_dataset = VcrEvalDataset(
        "test", eval_txt_db, img_db=eval_img_db, img_db_gt=eval_img_db_gt
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--txt_db",
        default="/txt/vcr_val.db/",
        type=str,
        help="The input train corpus. (LMDB)",
    )
    parser.add_argument(
        "--img_db",
        default="/img/vcr_gt_val/;/img/vcr_val/",
        type=str,
        help="The input train images.",
    )
    parser.add_argument("--split", default="val", type=str, help="The input split")
    parser.add_argument(
    ...
    parser.add_argument(
        "--batch_size", default=10, type=int, help="number of examples in a batch"
    )
