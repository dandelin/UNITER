...
def main(opts):
    ...
    # train_examples = None
    LOGGER.info(f"Loading Train Dataset {opts.train_txt_dbs}, " f"{opts.train_img_dbs}")
    # check multiple DBs
    ...
    # train
    LOGGER.info(f"Loading Train Dataset " f"{opts.train_txt_dbs}, {opts.train_img_dbs}")
    train_datasets = []
    for txt_path, img_path in zip(opts.train_txt_dbs, opts.train_img_dbs):
        img_db = all_img_dbs[img_path]
        txt_db = TxtTokLmdb(txt_path, opts.max_txt_len)
        train_datasets.append(ItmRankDataset(txt_db, img_db, opts.negative_size))
    ...
    # eval
    LOGGER.info(
        f"Loading val, test Dataset for full evaluation: "
    ...
    eval_dataset_test = ItmEvalDataset(
        test_txt_db, test_img_db, opts.inf_minibatch_size
    )
    eval_loader_test = build_dataloader(
        eval_dataset_test, itm_eval_collate, False, opts
    )
    ...
    model = UniterForImageTextRetrieval.from_pretrained(
        opts.model_config, state_dict=checkpoint, img_dim=IMG_DIM, margin=opts.margin
    )
    ...
    optimizer.step()
    while True:
        train_dataloader = build_dataloader(train_dataset, itm_rank_collate, True, opts)
        for step, batch in enumerate(train_dataloader):
            ...
            if (step + 1) % opts.gradient_accumulation_steps == 0:
                ...
                if global_step % 100 == 0:
                    ...
                    LOGGER.info(f"{tot_ex} examples trained at " f"{ex_per_sec} ex/s")
    ...
    # evaluation
    for split, loader in [("val", eval_loader_val), ("test", eval_loader_test)]:
        eval_log = evaluate(model, loader)
        TB_LOGGER.log_scaler_dict({f"eval/{split}_{k}": v for k, v in eval_log.items()})
        if hvd.rank() != 0:
            continue
        LOGGER.info(
            f"========================= {split} ===========================\n"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--checkpoint", default=None, type=str, help="pretrained MLM")
    ...
    parser.add_argument(
        ...
        default=128,
        type=int,
        help="Total batch size for training. " "(batch by examples)",
    )
    parser.add_argument(
        "--negative_size",
        default=1,
    ...
    parser.add_argument(
        "--margin", default=0.2, type=float, help="margin of ranking loss"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--optim", default="adam", choices=["adam", "adamax", "adamw"], help="optimizer"
    )
    parser.add_argument(
